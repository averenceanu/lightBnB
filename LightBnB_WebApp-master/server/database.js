 ...
 */
 const getAllProperties = function (options, limit = 10) {
  ...
  if (options.minimum_rating) {
    ...
    queryString += `AND WHERE (SELECT avg(rating) FROM property_reviews) >= $${queryParams.length}`;
  }
...
const addProperty = function(property) {
  ...
  const passedQueryParams = [property.owner_id, property.title, property.description, property.thumbnail_photo_url, property.cover_photo_url, property.cost_per_night, property.street, property.city, property.province, property.post_code, property.country, property.parking_spaces, property.number_of_bathrooms, property.number_of_bedrooms];
  
  pool.query(queryString, passedQueryParams).then((res) => res.rows);
